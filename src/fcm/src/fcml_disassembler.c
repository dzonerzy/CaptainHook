...
static int calculate_decoding_order(inst_decoding_def *decoding)
{
    ...
    /* REX/VEX/EVEX prefix. */
    if (FCML_DEF_PREFIX_W_1(prefixes) || FCML_DEF_PREFIX_W_0(prefixes) || FCML_DEF_PREFIX_L_1(prefixes) || FCML_DEF_PREFIX_L_0(prefixes) || FCML_DEF_PREFIX_L_prim_1(prefixes) || FCML_DEF_PREFIX_L_prim_0(prefixes))
    {
        order += 2;
    }
...
static fcml_bool inst_acceptor_prefixes(decoding_context *context)
{
    ...
    /* VEX required. */
    if ((FCML_DEF_PREFIX_VEX_REQ(decoding_def->prefixes_flags) && !prefixes->is_vex) || (!FCML_DEF_PREFIX_VEX_REQ(
                                                                                             decoding_def->prefixes_flags) &&
                                                                                         prefixes->is_vex))
    {
    ...
    /* XOP required. */
    if ((FCML_DEF_PREFIX_XOP_REQ(decoding_def->prefixes_flags) && !prefixes->is_xop) || (!FCML_DEF_PREFIX_XOP_REQ(
                                                                                             decoding_def->prefixes_flags) &&
                                                                                         prefixes->is_xop))
    {
    ...
    /* W field. */
    if ((FCML_DEF_PREFIX_W_0(decoding_def->prefixes_flags) && prefixes->W) || (FCML_DEF_PREFIX_W_1(decoding_def->prefixes_flags) && !prefixes->W))
    {
    ...
    if ((FCML_DEF_PREFIX_L_1(decoding_def->prefixes_flags) &&
        ...
        (FCML_DEF_PREFIX_L_0(decoding_def->prefixes_flags) && (!prefixes->is_avx || prefixes->L)))
    {
...
static fcml_bool inst_acceptor_addr_mode(decoding_context *context)
{
    ...
    return ((op_mode == FCML_OM_16_BIT || op_mode == FCML_OM_32_BIT) && FCML_DEF_OPCODE_FLAGS_16_32_BIT_MODE_SUPPORTED(
                                                                            inst_dec_def->opcode_flags)) ||
           (op_mode == FCML_OM_64_BIT && FCML_DEF_OPCODE_FLAGS_64_BIT_MODE_SUPPORTED(
                                             inst_dec_def->opcode_flags));
}
    ...
    do
    {
        ...
        if (result)
        {
                    ...
                    if (prefix_type != FCML_PT_GROUP_UNKNOWN)
                    {
                        ...
                        else
                        {
                            ...
                            case 0xC4:
                                if (prefix_type == FCML_PT_XOP &&
                                    FCML_VEX_MMMM(
                                        prefix_details->avx_bytes[0]) < 0x08)
                                {
                                    prefix_type = FCML_PT_GROUP_UNKNOWN;
                                    break;
                                }
            ...
            if (prefix_type != FCML_PT_GROUP_UNKNOWN)
            {
                ...
                prefix_details->mandatory_prefix = (p_flags & FCML_IDFPF_IS_MANDATORY_CANDIDATE)
                                                       ? FCML_TRUE
                                                       : FCML_FALSE;
                ...
                prefixes_details->is_avx = prefixes_details->is_vex || prefixes_details->is_evex || prefixes_details->is_xop;
                                       ...
                                       fcml_st_disassembler_result *result)
{
    ...
    if (!error)
    {
        ...
        fcml_fn_env_memory_copy(&inst_details->instruction_code,
                                context->code,
                                inst_details->instruction_size > FCML_INSTRUCTION_SIZE ? FCML_INSTRUCTION_SIZE : inst_details->instruction_size);
        ...
        if (decoding_context.pseudo_op)
        {
            ...
            inst->mnemonic = int_disasm->dialect_context->get_pseudo_op_mnemonic(decoding_context.pseudo_op);
            ...
            if (mnemonic)
            {
                ...
                inst->mnemonic =
                    int_disasm->dialect_context->render_mnemonic(
                        mnemonic->mnemonic, decoding_context.is_conditional ? &(decoding_context.condition) : NULL,
                        context->configuration.conditional_group,
                        context->configuration.carry_flag_conditional_suffix);
            }
            else
            {
                /* Mnemonic not found.*/
        ...
        fcml_fnp_disasm_postprocessor post_processor = int_disasm->dialect_context->disasm_postprocessor;
        if (post_processor)
        {
